...
const TYPE_EDIT_ROUTE: Record<string, string> = {
  "Purchase Return": "/purchase-return/edit",
};
...
export default function SupplierLedgerPage() {
  ...
  async function load() {
    setIsLoading(true);
    try {
      const res: any = await reportService.getSupplierLedger(id, selectedCompanyId, dateFrom, dateTo);
      setParty(res.party);
      setOpeningBalance(res.openingBalance || 0);
      setClosingBalance(res.closingBalance || 0);
      setEntries(res.entries || []);
    } catch (e: any) {
      toast.error(e.message || "Failed to load supplier ledger");
      setEntries([]);
    } finally {
      setIsLoading(false);
    }
    ...
    { key: "credit", header: "Credit", render: (r) => (r.credit ? money(r.credit) : "-"), className: "text-right" },
    {
      ...
      render: (r) => (
        <span className={r.balance < 0 ? "text-green-700 font-semibold" : "text-gray-900 font-semibold"}>
          {r.balance < 0 ? `${money(Math.abs(r.balance))} Adv` : `${money(r.balance)} Cr`}
        </span>
      ),
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <button onClick={() => router.push("/suppliers")} className="p-2 rounded-lg hover:bg-gray-100" title="Back to Suppliers">
          <ArrowLeft size={18} />
        </button>
        <div>
          <h1 className="text-lg font-bold text-gray-900">{party?.name || "Supplier"} — Ledger</h1>
      ...
      <div className="card p-4 flex flex-col sm:flex-row sm:items-end gap-4">
        ...
        <Button
          variant="outline"
          size="sm"
          onClick={() => reportService.exportSupplierLedger(id, selectedCompanyId, dateFrom, dateTo)}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="card p-4">
          ...
          <div className={`text-xl font-bold mt-1 ${openingBalance < 0 ? "text-green-700" : "text-gray-900"}`}>
            {openingBalance < 0 ? `${money(Math.abs(openingBalance))} Advance` : `${money(openingBalance)} Cr`}
          </div>
        </div>
        <div className="card p-4">
          ...
          <div className={`text-xl font-bold mt-1 ${closingBalance < 0 ? "text-green-700" : "text-red-600"}`}>
            {closingBalance < 0 ? `${money(Math.abs(closingBalance))} Advance` : `${money(closingBalance)} Payable`}
          </div>
        </div>
      </div>

      <p className="text-xs text-gray-500 -mt-2">
        Credit increases the balance owed to the supplier (Purchase); Debit decreases it (Purchase Return). Purchase invoices attribute to this supplier by
        their items (Purchase itself carries no supplier field) — this app doesn&apos;t track separate payment vouchers, so the balance reflects invoice
        activity, not individual payments made.
      </p>
