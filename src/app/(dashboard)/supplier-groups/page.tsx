...
export default function SupplierGroupsPage() {
  ...
  const [editingRecord, setEditingRecord] = useState<SupplierGroupRecord | null>(null);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormAlert(e.message || "Failed to save Supplier Group");
    }
  };

  const handleEditClick = (record: SupplierGroupRecord) => {
    setEditingRecord(record);
    setNameInput(record.name);
    setIsFormOpen(true);
  };

  const handleDeleteClick = (record: SupplierGroupRecord) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await supplierGroupService.deleteSupplierGroup(deletingRecord.id);
  ...
  const columns = [
    {
      key: "srno",
      header: "SR. NO.",
      accessor: (row: SupplierGroupRecord, index: number) => index + 1,
      className: "w-20 text-center font-semibold text-gray-500",
    },
    {
      key: "name",
      header: "GROUP NAME",
      accessor: (row: SupplierGroupRecord) => row.name,
  ...
  return (
    <div className="flex flex-col gap-6 w-full">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-gray-100 pb-5">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 tracking-tight">
            Supplier Groups (સપ્લાયર ગ્રુપ્સ લિસ્ટ)
          </h1>
          <p className="text-sm text-gray-500 mt-1">
            Group suppliers for procurement management and reporting.
          </p>
        </div>
      ...
      <Dialog
        ...
        title={editingRecord ? "Edit Supplier Group" : "Add Supplier Group"}
        footer={
          <>
      ...
      <ConfirmationDialog
        ...
        message={`Are you sure you want to delete the supplier group "${deletingRecord?.name}"? This action cannot be undone.`}
      />
    </div>
  );
}
