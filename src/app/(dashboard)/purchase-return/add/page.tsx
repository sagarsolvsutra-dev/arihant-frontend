...
// Fresh = green (sellable), Expired = amber (existing), Damaged = red (new, genuinely
// separate third state — see quirk history for why Expired used to be called Damaged
// internally; both are now real, independently-tracked stock buckets).
...
interface Line {
  ...
  // Damaged. Determines which bucket applyStockDelta decrements on the backend.
...
export default function AddPurchaseReturnPage() {
  ...
  // Set once an original invoice is fetched — a return can't predate the purchase
  // it's returning against. "YYYY-MM-DD", same shape DatePicker's minDate expects.
  ...
  // line ever joins the grid, instead of only surfacing after Save.
  ...
  // return can send different items back out of different godowns.
  ...
  // See purchase/add/page.tsx for why this fallback exists.
  ...
  // Looks up the original Purchase by invoice number and auto-fills Godown, a
  // Supplier suggestion (derived from the first line's Item.supplierId, since
  // Purchase itself never stores a real supplierId), and the full set of return
  // lines (defaulted to the original's full quantities — editable/removable below,
  // and the entry row above still works for adding extra freeform lines).
  const handleLookupInvoice = async () => {
    ...
    try {
      ...
      const matchedItem = items.find((i) => i._id === firstItemId);
      const suggestedSupplierId =
        matchedItem && (typeof matchedItem.supplierId === "string" ? matchedItem.supplierId : matchedItem.supplierId?._id);
      ...
      toast.error(err.message || "No purchase invoice found with that number");
    } finally {
      setLooking(false);
    }
  };
  ...
  // See purchase/add/page.tsx — negative means the supplier has refunded more than
  // this return is worth (a genuine excess, not an error); left unclamped, shown
  // as a clearly-labeled Excess Refund instead.
  ...
  const handleSave = async (e?: React.FormEvent) => {
    ...
    try {
      await purchaseReturnService.createPurchaseReturn({
        ...
        originalPurchaseId: originalPurchaseId || undefined,
        notes,
  ...
  return (
    <div className="mx-auto bg-[#f0f0f0] min-h-screen font-sans">
      <FormToolbar
        ...
        onSave={handleSave}
        isSaving={saving}
        onCancel={() => router.push("/purchase-return")}
        onClose={() => router.push("/purchase-return")}
      />
        ...
        <div className="flex-1 min-w-0 flex flex-col gap-4">
          ...
          <div className="border border-gray-300 bg-white p-4 rounded-md shadow-sm">
            <table className="w-full border-separate" style={{ borderSpacing: "0 10px" }}>
              <tbody>
                <tr>
                  <td className={rowLabel}>Original Invoice No.</td>
                  <td className="relative z-[66]">
                    <div className="flex items-center gap-2 w-72">
                      <Input
                        ...
                        onChange={(e) => {
                          ...
                          // leave `originalPurchaseId` pointing at the previously
                          // fetched invoice, so an edited-but-not-refetched number could
                          // save with a validateAgainstOriginal cap from the wrong
                          // invoice. Clearing it forces a fresh Fetch to re-link.
                          if (originalPurchaseId) setOriginalPurchaseId("");
                        }}
                        className={`${inputClass} flex-1`}
                        placeholder="Optional - auto-fills items below"
                      />
                ...
                <tr>
                  <td className={rowLabel}>Supplier <span className="text-red-500 font-bold">*</span></td>
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td className="relative z-[50]">
                    ...
                    <p className="text-[10px] text-gray-400 mt-1">Reason for the return, for your records — stock is always taken from regular (Fresh) stock either way.</p>
                  </td>
                </tr>
                <tr>
              ...
              <div className="p-4 space-y-4">
                ...
                <div>
                  <div className="flex items-center gap-1.5 text-[10px] text-gray-500 font-medium uppercase tracking-wide mb-2">
                    <Boxes size={12} />
                    Stock in this Godown (returning reduces this — Condition is a record of why, it does not change which stock is taken)
                  </div>
