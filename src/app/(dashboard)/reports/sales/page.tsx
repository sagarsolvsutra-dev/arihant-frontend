...
export default function SaleReportPage() {
  ...
  const [rows, setRows] = useState<SaleReportLine[]>([]);
  ...
  }, [selectedCompanyId, isContextLoading, page, search, dateFrom, dateTo, itemId, godownId, customerId]);

  async function load() {
    setIsLoading(true);
    try {
      const res: any = await reportService.getSaleReport({
      ...
      toast.error(e.message || "Failed to load sale report");
      setRows([]);
    } finally {
      setIsLoading(false);
    }
  ...
  const columns: Column<SaleReportLine>[] = [
    { key: "invoiceNo", header: "Invoice No", primary: true, render: (r) => (
        <button className="text-blue-600 hover:underline" onClick={() => router.push(`/sale/edit/${r.recordId}`)}>
          {r.invoiceNo}
        </button>
      ) },
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-lg font-bold text-gray-900">Sale Report</h1>
          <p className="text-xs text-gray-500 mt-0.5">Line-by-line sale detail, filterable by date, item, godown &amp; customer.</p>
        </div>
      </div>
